...
export class NetworkSettingsComponent {
  ...
  load() {
    this.backendService.getSettings().subscribe((resp) => {
      ...
      this.ethernet.isEnabled = resp.network_settings.ethernet_state === 'enabled';
      if (! this.wifiEnabled) {
        this.ipv4SettingsCurrent = {address: '', netmask: '', gateway: '', dns: ''};
      }
      ...
      this.allActive = resp.network_settings.active_connections.reduce((acc: any, conn: any) => {
        ...
        if (conn.type == 'wifi') {
          ...
          this.ethernet.ipv4SettingsCurrent = Object.assign({}, conn.ipv4_settings);
  ...
  onSubmitWifi() {
    ...
    if (this.ipv4_method === 'manual') {
      network_settings['network_settings']['ipv4_settings'] = this.ipv4_settings;
    }
    this.backendService.updateSettings(network_settings).subscribe((resp) => {this.load();});
  }
  ...
  onSubmitEthernet() {
    ...
    if (this.ethernet.ipv4Method === 'manual') {
      network_settings['network_settings']['ipv4_settings'] = this.ethernet.ipv4Settings;
    }
    this.backendService.updateSettings(network_settings).subscribe((resp) => {this.load();});
  }
...
class Ehternet {
  ...
  ipv4Method : string = '';
  ipv4SettingsCurrent : Ipv4Settings = {address: '', netmask: '', gateway: '', dns: ''};
  ipv4Settings : Ipv4Settings = {address: '', netmask: '', gateway: '', dns: ''};
